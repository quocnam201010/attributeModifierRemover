...
public final class AttributeModifierRemover extends JavaPlugin {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (targetSelector.startsWith("@")) {
            targets = Bukkit.selectEntities(sender, targetSelector).stream()
                .filter(e -> e instanceof Player)
                .map(e -> (Player) e)
                .toList();
            if (targets.isEmpty()) {
                sender.sendMessage("No players found matching selector: " + targetSelector);
                return true;
            }
        } else {
            Player target = Bukkit.getPlayerExact(targetSelector);
            if (target == null) {
                sender.sendMessage("Player not found or not online: " + targetSelector);
                return true;
            }
        ...
        for (Player target : targets) {
            ...
            if (instance == null) {
                sender.sendMessage("Player " + target.getName() + " does not have attribute instance: " + attributeArg);
                continue;
            }
        ...
        if (targets.size() > 1) {
            sender.sendMessage("Total modifiers removed: " + totalRemoved + " across " + targets.size() + " players");
        }
